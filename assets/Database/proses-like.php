...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    require_once 'koneksi.php'; // Sesuaikan path koneksi dengan struktur aplikasi Anda
    ...
    if ($recipe_id > 0) {
        ...
        if ($result_check->num_rows > 0) {
            // Pengguna sudah menyukai resep ini sebelumnya
            echo "Anda sudah menyukai resep ini.";
        } else {
            ...
            if ($stmt_insert->execute()) {
                // Redirect kembali ke halaman sebelumnya setelah like berhasil
                if(isset($_SERVER['HTTP_REFERER'])) {
                    header("Location: " . $_SERVER['HTTP_REFERER']);
                } else {
                    header("Location: ../../tampilan/halaman-awal.php"); // Redirect ke halaman default jika HTTP_REFERER tidak tersedia
                }
                exit();
            } else {
